...
model DP_pT "Lumped, 3-terms pressure drop model, pT formulation"
  ...
  /*********** PORTS ****************/
  Interfaces.Fluid.FlangeA_pT InFlow(redeclare package Medium =
        Medium)
    annotation (Placement(transformation(extent={{-100,-10},{-80,10}})));
  Interfaces.Fluid.FlangeB_pT OutFlow(redeclare package Medium =
               Medium)
  ...
  parameter Modelica.Units.SI.Area A=10e-5
    "Valve throat area for quadratic pressure drop (dp = 1/A²*M_dot²/(2*rho)) - Set 5e3 to put it to zero"
    annotation (Dialog(enable=(not UseNom)));
  Modelica.Units.SI.Length h_ok;
  Real k_ok;
  Modelica.Units.SI.Area A_ok;
...
//           T_nom,fill(0,0)) "Nominal density"    annotation (Dialog(tab="Nominal Conditions"));
  parameter Modelica.Units.SI.Density rho_nom=Medium.density(
      Medium.setState_pTX(
      p_nom,
      T_nom,
  ...
  Modelica.Units.SI.Density d_fl(start=rho_nom);
  Medium.Density rho(start=rho_nom);
  Modelica.Units.SI.Pressure p_fl;
  Medium.Temperature T_fl;
  Modelica.Units.SI.Pressure DELTAp(start=DELTAp_start);
  Modelica.Units.SI.Pressure DELTAp_quad(start=DELTAp_quad_nom)
    "Quadratic pressure drop";
  Modelica.Units.SI.Pressure DELTAp_lin(start=DELTAp_lin_nom)
    "Linear pressure drop";
  Modelica.Units.SI.Pressure DELTAp_stat(start=DELTAp_stat_nom)
  ...
  Modelica.Units.SI.MassFlowRate Mdot_0 "Nominal mass flow rate";
